...
To test the code generated by Copilot, first run the `xpm run test -C tests` command in a terminal. This will execute the test suite natively and verify that the code behaves as expected. If successful, run the `xpm run test-ci -C tests` command. This will execute more cross tests. Make sure to write comprehensive test cases to cover various scenarios and edge cases.
