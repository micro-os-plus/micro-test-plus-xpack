...
diff -u \
  <(grep -v -e '^# Running:' -e '^# Built with ' -e '^Built with ' "$1" | sed -e 's|0[xX][1-9a-fA-F][0-9a-fA-F]*|0xXXX|g' -e 's|[0-9a-fA-F]\{16\}|0xXXX|g' -e 's|, time: [0-9]*\.[0-9]* ms||g' -e 's|  line: [0-9][0-9]*|  line: NNN|g') \
  <(grep -v -e '^# Running:' -e '^# Built with ' -e '^Built with ' "$2" | sed -e 's|0[xX][1-9a-fA-F][0-9a-fA-F]*|0xXXX|g' -e 's|[0-9a-fA-F]\{16\}|0xXXX|g' -e 's|, time: [0-9]*\.[0-9]* ms||g' -e 's|  line: [0-9][0-9]*|  line: NNN|g')
