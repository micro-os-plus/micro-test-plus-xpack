...
- `src/hooks.cpp` — the `micro_os_plus_startup_post_init_array_hook`/
  ...
  `activity_led` (`platform::led_green`), turning it on/off
  around the application's run. `led_green` has no data members and a
  ...
  the C++ runtime is fully up, to touch peripherals. No
