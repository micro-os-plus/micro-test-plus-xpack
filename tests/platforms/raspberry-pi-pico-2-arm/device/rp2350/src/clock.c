...
// PLL_SYS_VCO_FREQ_HZ/POSTDIV1/POSTDIV2 in the SDK's `hardware/clocks.h`
// for XOSC_HZ==12 MHz && SYS_CLK_HZ==150 MHz).
//
...
// (hardware_xosc/hardware_pll). Only clk_ref, clk_sys and clk_peri are
// configured; clk_usb/clk_adc/clk_hstx/clk_rtc and the tick generators
// are not used by this platform and are left at their reset state.

// PLL_SYS: 12 MHz / 1 * 125 = 1500 MHz VCO / 5 / 2 = 150 MHz.
#define PLL_SYS_REFDIV (1u)
#define PLL_SYS_FBDIV (125u)
#define PLL_SYS_POSTDIV1 (5u)
...
micro_os_plus_rp2350_clock_init (void)
{
  ...
  // before reconfiguring the PLL and the aux muxes, to avoid glitches.
  ...
  while (!(CLOCKS->CLK_SYS_SELECTED
    ...
    {
      // Busy wait for the glitchless mux to settle on the aux (PLL_SYS).
    }
...
micro_os_plus_rp2350_clock_get_frequency_hz (void)
{
  ...
  // programmed above; nothing else in this codebase changes the source
  // afterwards.
  uint32_t refdiv = PLL_SYS->CS & PLL_CS_REFDIV_BITS;
  uint32_t fbdiv_int = PLL_SYS->FBDIV_INT & PLL_FBDIV_INT_BITS;
  uint32_t postdiv1
      = (PLL_SYS->PRIM & PLL_PRIM_POSTDIV1_BITS) >> PLL_PRIM_POSTDIV1_LSB;
  uint32_t postdiv2
      = (PLL_SYS->PRIM & PLL_PRIM_POSTDIV2_BITS) >> PLL_PRIM_POSTDIV2_LSB;
  uint32_t clk_sys_div_int
      = (CLOCKS->CLK_SYS_DIV & CLOCKS_CLK_SYS_DIV_INT_BITS)
        >> CLOCKS_CLK_SYS_DIV_INT_LSB;

  uint32_t vco_hz = (XOSC_HZ / refdiv) * fbdiv_int;
  uint32_t pll_hz = vco_hz / (postdiv1 * postdiv2);
