...
{
  // Install the Direct-mode trap handler (device/src/trap-handler.cpp)
  // before anything else runs, as a safety net; see
  // micro-os-plus/trap-handler.h for why this platform needs no more
  // than that.
  device::install_trap_handler ();

  system_clock.initialise ();
...
{
  // On this toolchain's rv32/ilp32 ABI, `uint32_t` is `unsigned long`, not
  // `unsigned int`; cast explicitly rather than switch to `%lu`, to keep
  // the same format string as the other Pico platforms' hooks.cpp.
  trace::printf (
      "SystemCoreClock: %u Hz\n",
      static_cast<unsigned int> (system_clock.clock_get_frequency_hz ()));
