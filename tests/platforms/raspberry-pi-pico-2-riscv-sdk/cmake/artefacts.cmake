...
prepend_compile_common_private_options (bs2_default_library)

# Unlike the Arm sibling, `bs2_default` (the boot-stage-2 executable itself, not
# just `bs2_default_library`) also needs the platform's march/mabi flags: the
# default boot2 source (`boot2_w25q080.S`, included from
# `compile_time_choice.S`) uses the `Zbs` bit-manipulation instruction `bclri`,
# which the toolchain's default march does not enable.
prepend_compile_common_private_options (bs2_default)
...
# The generic `@xpack-dev-tools/openocd` package used by every other platform
# (pinned at 0.12.0-7.1, the latest published release) does not bundle
# `target/rp2350-riscv.cfg` (RP2350 RISC-V/Hazard3 core support is not yet in
# its script set). Use the Pico SDK's own OpenOCD install instead, the same one
# the VS Code debug launcher (`.vscode/launch.json`) already uses for this
# platform, which does have it.
if ("${CMAKE_HOST_SYSTEM_NAME}" STREQUAL "Windows")
  set (xpack_pico_sdk_openocd
       "$ENV{USERPROFILE}/.pico-sdk/openocd/0.12.0+dev/openocd.exe"
  )
else ()
  set (xpack_pico_sdk_openocd "$ENV{HOME}/.pico-sdk/openocd/0.12.0+dev/openocd")
endif ()
