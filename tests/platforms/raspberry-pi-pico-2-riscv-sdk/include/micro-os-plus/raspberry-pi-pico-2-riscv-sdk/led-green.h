...
#ifndef MICRO_OS_PLUS_RASPBERRY_PI_PICO_2_RISCV_SDK_LED_GREEN_H_
#define MICRO_OS_PLUS_RASPBERRY_PI_PICO_2_RISCV_SDK_LED_GREEN_H_
...
namespace platform
{
  ...
  // `hardware_gpio` driver (`hardware/gpio.h`); the GPIO API is the same
  // regardless of which RP2350 core (Arm or, here, the Hazard3 RISC-V
  // core) it runs on. This platform already links the Pico SDK proper
  // (pico_crt0/pico_runtime_init/pico_standard_link), so there is no
  // reason to bypass its GPIO driver here.
  class led_green
  {
  public:
    led_green () = default;
...
#endif // MICRO_OS_PLUS_RASPBERRY_PI_PICO_2_RISCV_SDK_LED_GREEN_H_
